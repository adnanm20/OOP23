...
TEST_CASE("Expects to initialize MojNizInt using initializer_list") {
  ...
  for (auto i = 1; i <= 5; ++i) CHECK_EQ(mojNiz.at(i - 1), i);
  for (auto i = 0; i >= -5; --i) CHECK_EQ(mojNiz.at(std::abs(i) + 5), i);
}
...
TEST_CASE("Expects to have const access to at and size method") {
  ...
  for (auto i = 0; i < 5; ++i) CHECK_EQ(mojConstNiz.at(i), i + 1);
}
...
TEST_CASE(
  ...
  for (auto i = 4; i < 6; ++i) CHECK_EQ(mojNiz.at(i), i + 1);
}
...
TEST_CASE(
    "Expects to throw when attempting to add two vectors together with "
    "different lengths") {
...
TEST_CASE(
    "FIRST_PUSH_BACK_Expects push_back to append element at the end of "
    " array") {
  MojNizInt mojNiz;
  mojNiz.push_back(5);
